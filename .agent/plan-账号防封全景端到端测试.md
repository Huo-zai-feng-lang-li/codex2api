...
- 当前最新服务 PID 33776，监听 `127.0.0.1:18080` 与 `127.0.0.1:1455`，运行二进制 SHA256 `CB592B25E060D241491D14571237438E823FBDF5809FECDB5C30BA63FDB08044`。
...
### 阶段 4：真实上游 E2E（本轮已执行，部分通过）

- 真实 HTTP 机器人首包与续问包均 HTTP 200、`completed`，`previous_response_id` 续链可用。
- 本地 WS 入站握手返回 101，但测试窗口内未收到终止事件；日志与 usage 记录出现上游 `websocket_unsupported`/400、账号 652 流提前结束 598，实际走 HTTP fallback。
- `/health` 保持 `status=ok`、可用账号 6/45；运行态续链持久化失败计数升至 23，出现 `context deadline exceeded`。
- 修复后独立公网 WSS 101/ALPN 探针仍通过；该证据只覆盖公开回显服务，不等同于当前业务上游 WSS 闭环。
...
- 当前判定：HTTP 路径通过；业务 WSS 与续链持久化为未闭环项，暂不标记全链路通过。
