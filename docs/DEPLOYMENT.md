...
如果你使用的是已经构建好的 Windows 本地目录，日常启动请使用：

```text
桌面快捷方式 -> start-codex2api.bat -> codex2api.exe
```

也可以直接双击项目根目录下的 `start-codex2api.bat`。不建议直接双击 `codex2api.exe`，因为启动脚本会额外完成以下工作：

- 自动切到项目目录，保证数据库、日志、前端资源路径正确。
- 设置 `CODEX_PORT=18080` 和 `CODEX_BIND=127.0.0.1`。
- 检查服务是否已经运行，没运行才启动，避免重复启动。
- 自动打开管理后台 `http://127.0.0.1:18080/admin/`。
- 把启动日志写入 `logs/` 目录。

迁移到另一台电脑或更换目录后，复制整个项目目录，再双击 `create-desktop-shortcut.bat` 重新生成桌面快捷方式。
